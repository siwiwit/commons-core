...
public abstract class SimpleDataGridPagedResultExecutorTemplate<CORETABLECLASS> {
	...
	protected static final SimpleDateFormat DATE_FORMATTER = new SimpleDateFormat(PagedSimpleDataGridData.DATE_TO_STRING_SERIALIZATION_PATTERN);
	...
	public PagedSimpleDataGridData executeQuery(int pageSize , int page)throws Exception{
		Integer cnt = count() ; 
		if ( cnt==null||cnt.intValue()==0)
			return null ; 
		PagedSimpleDataGridData retval = new PagedSimpleDataGridData();
		int firstRowPosition =  retval.adjustPagination(page, pageSize, cnt);
		List<Object[]> theData =  list(getActualFieldToFetch(), pageSize, firstRowPosition);
		retval.pushData(ExtendedBeanUtils.getInstance().readPropertyTypeAsArray(getCoreDataClass(), getActualFieldToFetch()), new DataIDLocator() {
			
			@Override
			public Object getDataIDObject(Object[] dataArrayy) {
				return dataArrayy[getIdFieldIndex()];
			}
		}, theData, getActualFieldToFetch(), DATE_FORMATTER_AS_WRAPPED);
		return retval ; 
	}
